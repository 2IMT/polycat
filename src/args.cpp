...
const std::string args::CONF_PATH_DEFAULT = "polycat-config.json";
...
const char* args::parse_err::what() const noexcept { return m_message.c_str(); }

args::args(int argc, char** argv) noexcept :
    m_argc(argc),
    m_argv(argv),
    m_stat_path(STAT_PATH_DEFAULT),
    m_conf_path(CONF_PATH_DEFAULT)
{
}
...
void args::parse()
{
    ...
    program.add_argument(CONF_PATH_ARG, CONF_PATH_ARG_LONG)
        ...
        .default_value(CONF_PATH_DEFAULT);
